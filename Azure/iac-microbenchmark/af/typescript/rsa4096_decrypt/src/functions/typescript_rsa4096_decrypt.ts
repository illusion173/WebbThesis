...
dotenv.config();
export async function rsa4096_decrypt_handler(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    const keyName = process.env.RSA4096_KEY_NAME;
...
app.http('typescript_rsa4096_decrypt', {
    methods: ['POST'],
    authLevel: 'anonymous',
    handler: rsa4096_decrypt_handler
});
