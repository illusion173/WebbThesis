...
namespace dotnet_ecc256_verify
{
    ...
    public class ecc256_verify_program
    {
        private readonly ILogger<ecc256_verify_program> _logger;

        public ecc256_verify_program(ILogger<ecc256_verify_program> logger)
        {
            _logger = logger;
        }

        [Function("dotnet_ecc256_verify_program")]
        public static async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequest req)
        {
            ...
            try
            {
                ...
                var requestModel = JsonSerializer.Deserialize<ecc256_verifyRequest>(requestBodyStr, options);
                ...
                try
                {
                    ...
                    VerifyResult verifyResult = await cryptoClient.VerifyAsync(SignatureAlgorithm.ES256, messageDigestBytes, signatureBytes);

                    return new OkObjectResult(new ecc256_verifyResponse { is_valid = verifyResult.IsValid });
                }
                catch (Exception ex)
                {
