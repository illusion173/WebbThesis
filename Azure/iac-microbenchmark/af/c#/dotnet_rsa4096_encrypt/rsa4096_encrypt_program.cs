...
namespace dotnet_rsa4096_encrypt
{
    // Struct representing the incoming request
    public struct rsa4096_encryptRequest
    {
        public string message { get; set; }
    }

    // Struct representing the outgoing response
    public struct rsa4096_encryptResponse
    {
    ...
    public class rsa4096_encrypt_program
    {
        private readonly ILogger<rsa4096_encrypt_program> _logger;

        public rsa4096_encrypt_program(ILogger<rsa4096_encrypt_program> logger)
        {
            _logger = logger;
        }

        [Function("dotnet_rsa4096_encrypt_program")]
        public static async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequest req)
        {
            string? RSA4096_KEY_NAME = Environment.GetEnvironmentVariable("RSA4096_KEY_NAME");
            string? AZURE_KEY_VAULT_URL = Environment.GetEnvironmentVariable("AZURE_KEY_VAULT_URL");

            if (string.IsNullOrEmpty(RSA4096_KEY_NAME) || string.IsNullOrEmpty(AZURE_KEY_VAULT_URL))
            {
            ...
            try
            {
                ...
                var requestModel = JsonSerializer.Deserialize<rsa4096_encryptRequest>(requestBodyStr, options);
                ...
                try
                {
                    ...
                    KeyVaultKey key = await keyClient.GetKeyAsync(RSA4096_KEY_NAME);
                    ...
                    return new OkObjectResult(new rsa4096_encryptResponse
                    {
