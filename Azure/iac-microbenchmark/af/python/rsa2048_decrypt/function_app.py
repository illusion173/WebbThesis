...
@app.function_name(name="python_rsa2048_decrypt")
@app.route(route="rsa2048_decrypt")
def main(req: func.HttpRequest) -> func.HttpResponse:

    # Get environment variables
    key_vault_url = os.environ["AZURE_KEY_VAULT_URL"]
    key_name = os.environ["RSA2048_KEY_NAME"]

    body = req.get_json()

    if not body:
        return func.HttpResponse(f"ERROR: HTTP request for rsa2048_decrypt is missing body.", status_code=400)
    ...
    try:
        ...
        return func.HttpResponse(f"ERROR: HTTP request for an error for operation rsa2048_decrypt {e} .", status_code=500)
