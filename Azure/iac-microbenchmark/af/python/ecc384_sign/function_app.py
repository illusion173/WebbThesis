...
@app.function_name(name="python_ecc384_sign")
@app.route(route="ecc384_sign")
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not body:
        return func.HttpResponse(f"ERROR: HTTP request for ecc384_sign is missing body.", status_code=400)
    ...
    try:
        ...
        sign_result = crypto_client.sign(
            algorithm=SignatureAlgorithm.es384,  # Specify the signing algorithm
            digest=message_digest_bytes  # Pass the SHA-384 hash
        )
        ...
        return func.HttpResponse(f"ERROR: HTTP request for an error for operation ecc384_sign {e} .", status_code=500)
