...
struct ECC384Request {
    message: String,
    signature: String,
}

#[derive(Serialize, Deserialize, Debug)]
struct ECC384Response {
    valid: bool,
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let ecc_kms_key_id =
        env::var("ECC384_KMS_KEY_ARN").map_err(|_| "ECC 384 KMS key ARN not set?")?;
    ...
    let message_struct: ECC384Request = serde_json::from_str(body_str)
...
async fn kms_client_verify_message(
    ...
    message: &str,
    signature_b64: &str,
) -> Result<ECC384Response, Box<dyn Error>> {
    // Decode the base64-encoded signature
    let signature = decode(signature_b64).map_err(|_| "Failed to decode signature ECC384")?;
    ...
    let verify_response = ECC384Response { valid: is_valid };
