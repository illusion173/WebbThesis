...
namespace Program
{
    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            if (string.IsNullOrEmpty(RSA4096_KEY_NAME))
            {
                Console.WriteLine("{ \"Error\": \"Environment variable RSA4096_KEY_NAME is not set.\" }");
                return;
            }
            ...
            try
            {
                ...
                var requestModel = JsonSerializer.Deserialize<rsa4096_decryptRequest>(args[0], options);
                ...
                try
                {
                    ...
                    Console.WriteLine(JsonSerializer.Serialize(new rsa4096_decryptResponse { message = decryptedMessage }));
                }
                catch (Exception ex)
                {
                    Console.WriteLine("{\"Error\": \"Error doing Operation rsa4096_decrypt: " + ex.Message + "\"}");
                }
            }
            catch (JsonException)
            {
