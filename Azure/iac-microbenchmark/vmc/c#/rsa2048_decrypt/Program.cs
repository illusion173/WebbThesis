...
namespace Program
{
    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            if (string.IsNullOrEmpty(RSA2048_KEY_NAME))
            {
                Console.WriteLine("{ \"Error\": \"Environment variable RSA2048_KEY_NAME is not set.\" }");
                return;
            }
            ...
            try
            {
                ...
                try
                {
                    ...
                    Console.WriteLine(JsonSerializer.Serialize(new rsa2048_decryptResponse { message = decryptedMessage }));
                }
                catch (Exception ex)
                {
                    Console.WriteLine("{\"Error\": \"Error doing Operation rsa2048_decrypt: " + ex.Message + "\"}");
                }
            }
            catch (JsonException)
            {
