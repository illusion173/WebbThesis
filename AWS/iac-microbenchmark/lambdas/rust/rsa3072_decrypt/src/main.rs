...
use serde_json::json;
use std::env; // OpenSSL for decryption
...
async fn function_handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    let rsa_kms_key_id = env::var("RSA3072_KMS_KEY_ARN")
        .map_err(|_| Error::from("RSA 3072 KMS key ARN not set?"))?;
    ...
    let request_struct: RSA3072DecryptRequest = match serde_json::from_str(body_str) {
    ...
    match aws_kms_rsa_decrypt(&kms_client, &rsa_kms_key_id, request_struct).await {
        Ok(decrypted_message) => {
            let response_body = json!(RSA3072DecryptResponse {
