...
async fn function_handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    let ecc_kms_key_id =
        env::var("ECC384_KMS_KEY_ARN").map_err(|_| Error::from("ECC 384 KMS key ARN not set?"))?;
    ...
    let message_struct: ECC384Request = match serde_json::from_str(body_str) {
