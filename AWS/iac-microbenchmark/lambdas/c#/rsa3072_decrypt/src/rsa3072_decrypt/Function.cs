    ...
    public class Function
    {
        ...
        public async Task<APIGatewayHttpApiV2ProxyResponse> FunctionHandler(APIGatewayHttpApiV2ProxyRequest request, ILambdaContext context)
        {
            ...
            try
            {
                ...
                var requestModel = JsonSerializer.Deserialize<rsa3072_decryptRequest>(request.Body, options);
                ...
                var responseModel = new rsa3072_decryptResponse
                {
                    Message = plaintext,
                };
            ...
            {
                // Log the exception and return a 500 error response
                context.Logger.LogError($"Error processing request for rsa3072_decrypt c#: {ex.Message}");
                var errorRsp = new Dictionary<string, string>
            {
                { "Error", $"Error processing request for rsa3072_decrypt c#: {ex.Message}" }
            };
