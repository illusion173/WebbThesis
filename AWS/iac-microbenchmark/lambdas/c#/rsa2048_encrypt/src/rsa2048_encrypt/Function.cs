    ...
    public APIGatewayProxyResponse FunctionHandler(APIGatewayProxyRequest request, ILambdaContext context)
    {
      // Build APIGW response
        ...
        try
        {
            ...
            var responseModel = new rsa2048_encryptResponse
            {
        ...
        {
            // Log the exception and return a 500 error response
            context.Logger.LogError($"Error processing request for rsa2048_encrypt c#: {ex.Message}");
            var errorRsp = new Dictionary<string, string>
            {
                { "Error", $"Error processing request for rsa2048_encrypt c#: {ex.Message}" }
            };
