    ...
    public class Function
    {
        ...
        public async Task<APIGatewayHttpApiV2ProxyResponse> FunctionHandler(APIGatewayHttpApiV2ProxyRequest request, ILambdaContext context)
        {
            ...
            try
            {
                ...
                var requestModel = JsonSerializer.Deserialize<ecc256_verifyRequest>(request.Body, options);
                ...
                var verifyRequest = new VerifyRequest
                {
                    ...
                    SigningAlgorithm = SigningAlgorithmSpec.ECDSA_SHA_256 // Use ECDSA_SHA_384 for P-384
                };
                ...
                var responseModel = new ecc256_verifyResponse
                {
                    isValid = true,
                };
            ...
            {
                // Log the exception and return a 500 error response
                context.Logger.LogError($"Error processing request for ecc256_verify c#: {ex.Message}");
                var errorRsp = new Dictionary<string, string>
            {
                { "Error", $"Error processing request for ecc256_verify c#: {ex.Message}" }
            };
