    ...
    public class Function
    {
        ...
        public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest request, ILambdaContext context)
        {
            ...
            try
            {
                ...
                var requestModel = JsonSerializer.Deserialize<rsa2048_decryptRequest>(request.Body, options);
                ...
                var responseModel = new rsa2048_decryptResponse
                {
                    Message = plaintext,
                };
            ...
            {
                // Log the exception and return a 500 error response
                context.Logger.LogError($"Error processing request for rsa2048_decrypt c#: {ex.Message}");
                var errorRsp = new Dictionary<string, string>
            {
                { "Error", $"Error processing request for rsa2048_decrypt c#: {ex.Message}" }
            };
