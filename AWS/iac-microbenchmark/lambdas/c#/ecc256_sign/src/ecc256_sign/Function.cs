    ...
    public class Function
    {
        ...
        public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest request, ILambdaContext context)
        {
            ...
            try
            {
                ...
                var responseModel = new ecc256_signResponse
                {
            ...
            {
                // Log the exception and return a 500 error response
                context.Logger.LogError($"Error processing request for ecc256_sign c#: {ex.Message}");
                var errorRsp = new Dictionary<string, string>
            {
                { "Error", $"Error processing request for ecc256_sign c#: {ex.Message}" }
            };
